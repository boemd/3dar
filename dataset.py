...
def homographyGeneration(raw_image, image_path, data_path, pairs_per_img):
    """
    :param raw_image: input image
    :param image_path: input image to crop
    :param data_path: path in which to save the produced files
    :param pairs_per_img: number of samples that have to be computed for each input image
    """
    ...
    while i < pairs_per_img + 1:
        y_start = random.randint(rho, y_max)
        y_end = y_start + patch_size
        x_start = random.randint(rho, x_max)
        ...
        y_1_offset = random.randint(-1*rho, rho)
        x_1_offset = random.randint(-1*rho, rho)
        y_2_offset = random.randint(-1*rho, rho)
        x_2_offset = random.randint(-1*rho, rho)
        y_3_offset = random.randint(-1*rho, rho)
        x_3_offset = random.randint(-1*rho, rho)
        y_4_offset = random.randint(-1*rho, rho)
        x_4_offset = random.randint(-1*rho, rho)
        ...
        if not [y_1,x_1,y_2,x_2,y_3,x_3,y_4,x_4] in random_list:
            ...
            img_patch_path = os.path.join(data_path, (raw_image.split('.')[0] + '_' + str(i) + '_1' + '.jpg'))
            cv2.imwrite(img_patch_path, img_patch)
            img_perburb_patch_path = os.path.join(data_path, (raw_image.split('.')[0] + '_' + str(i) + '_2' + '.jpg'))
            cv2.imwrite(img_perburb_patch_path, img_perburb_patch)
            txt_re = os.path.join(data_path, (raw_image.split('.')[0] + '_' + str(i) + '_re' + '.txt'))
            txt_ab = os.path.join(data_path, (raw_image.split('.')[0] + '_' + str(i) + '_ab' + '.txt'))
            txt_hm = os.path.join(data_path, (raw_image.split('.')[0] + '_' + str(i) + '_hm' + '.txt'))
...
def main():
    """
    adapted from the repo https://github.com/linjian93/tensorflow_homographynet
    """
    ...
    parser.add_argument('--rawTestDataPath', type=str, default='../coco_test', help='The raw data path.')
    parser.add_argument('--testDataPath', type=str, default='../coco_test', help='The test data path')
    parser.add_argument('--testDataNumber', type=int, default=598, help='The data size for testing')
